...
package org.denivip.osmf.net.httpstreaming.hls
{
	...
	public class HTTPHLSStreamSource implements IHTTPStreamSource, IHTTPStreamHandler
	{
		...
		{
            _source_servers = resource.getMetadataValue("source_servers") as Array;    
            if (_source_servers == null) {
                _source_servers = new Array();
                
                var url:URL = new URL((resource as URLResource).url);
                var source_server:String = url.host;
                if (url.port) {
                   source_server += ":" + url.port; 
                }
                
                _source_servers.push(source_server);
            }
            
            _rtmfp_url = resource.getMetadataValue("rtmfp_url") as String;
            _rtmfp_url_peers = resource.getMetadataValue("rtmfp_url_peers") as Number;
            
            _index_rtmfp_url = resource.getMetadataValue("index_rtmfp_url") as String;
            _index_rtmfp_url_peers = resource.getMetadataValue("index_rtmfp_url_peers") as Number;
            
            _http_tracker = resource.getMetadataValue('http_tracker') as String;
            _remote_log_base_url = resource.getMetadataValue('remote_log_base_url') as String;
					...
					switch(_request.kind)
					{
						...
						case HTTPStreamRequestKind.BEST_EFFORT_DOWNLOAD:
							...
							if (_downloader == null)
							{
                                ...
                                for (var k:int; k < _source_servers.length; k++) {
                                    servers.push(_source_servers[k]);   
                                }

                                var targetDuration:Number = (_indexHandler as HTTPStreamingHLSIndexHandler).getTargetDuration(_qualityLevel);
                                var additionalInfo:Object = new Object();
                                additionalInfo['http_tracker'] = _http_tracker;
                                additionalInfo['remote_log_base_url'] = _remote_log_base_url;
                                _downloader = new P2PLoader(_rtmfp_url, _rtmfp_url_peers, servers, _dispatcher, additionalInfo, false, targetDuration);
                                _downloader.addEventListener(P2PLoaderStatusEvent.ON_STATUS, onP2PDownloaderStatus);
                                _downloader.setIndexData(_lastIdxData);
							}
		...
		private function onRequestLoadIndex(event:HTTPStreamingIndexHandlerEvent):void
		{
			...
			if (_currentIndexDownloadEvent == null)
			{
                ...
                if (_indexDownloader == null) {
                   ...
                   _indexDownloader = new P2PLoader(_index_rtmfp_url, _index_rtmfp_url_peers, servers, _dispatcher);
                }
                */
                if (_downloader != null && _downloader.canGetIdx())
                {
                    _downloader.getIdx(_currentIndexDownloadEvent.request);
                }
                else {
		...
		private function onIndexComplete(event:HTTPStreamingEvent):void
		{
            ...
            _lastIdxData = bytes;
            if (_downloader) {
               _downloader.setIndexData(bytes); 
            }
        ...
        private function onP2PDownloaderStatus(event:P2PLoaderStatusEvent):void {
            if (event.code == P2PLoaderStatusEvent.IDX_GOT)
            {
                var streamingEvent:HTTPStreamingEvent = new HTTPStreamingEvent(
                    ...
                    event.request.url, // urlString
                    event.idxData.length, // bytesDownloaded
                    HTTPStreamingEventReason.NORMAL, // reason
                    null /*this*/); // downloader
                    
                _dispatcher.dispatchEvent(streamingEvent);
                
                _indexHandler.processIndexData(event.idxData, _currentIndexDownloadEvent.requestContext);
			    processPendingIndexLoadingRequest();
            }
        }
        ...
        private var _downloader:P2PLoader = null;
        private var _lastIdxData:ByteArray = null;
        ...
        //private var _indexDownloader:P2PLoader = null;
        ...
        private var _source_servers:Array;
        private var _rtmfp_url:String;
        private var _rtmfp_url_peers:Number;
        private var _index_rtmfp_url:String;
        private var _index_rtmfp_url_peers:Number;
        private var _http_tracker:String;
        private var _remote_log_base_url:String;
