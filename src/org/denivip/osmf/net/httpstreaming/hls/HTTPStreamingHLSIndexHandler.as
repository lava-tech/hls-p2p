 ...
 package org.denivip.osmf.net.httpstreaming.hls
{
	...
	public class HTTPStreamingHLSIndexHandler extends HTTPStreamingIndexHandlerBase
	{
        ...
        private var _streamLastLoadTimes:Array;
        private var _streamLastDatas:Array;
        private var _streamReloadFactor:Array;
        ...
        public function getTargetDuration(quality:int):Number {
           return _rateVec[quality].targetDuration; 
        }
		...
		override public function initialize(args:Object):void{
			...
			for each(var hsi:HLSStreamInfo in _indexInfo.streams){
                ...
                _streamLastLoadTimes.push( -1);
                _streamLastDatas.push(null);
                _streamReloadFactor.push(1);
		...
		override public function dispose():void{
            ...
            _streamLastLoadTimes = null;
            _streamLastDatas = null;
            _streamReloadFactor = null;
			_streamURLs = null;
        ...
        */
		...
		override public function processIndexData(data:*, indexContext:Object):void{
			...
			if(getQualifiedClassName(indexContext) == "Array") {
                ...
                if (_streamLastDatas[quality] != String(data)) {
                    _streamReloadFactor[quality] = 1;
                }
                else {
                   _streamReloadFactor[quality] = 0.5; 
                }
                
                _streamLastLoadTimes[quality] = getTimer();
                
                _streamLastDatas[quality] = String(data);
		...
		override public function getNextFile(quality:int):HTTPStreamRequest{
			...
			if(item.isLive){
				...
				if(_segment >= manifest.length){ // Try to force a reload
                    ...
                    var streamTime:int = _streamLastLoadTimes[quality];
                    if (streamTime != -1) {
                       if ((now - streamTime) > (item.targetDuration * 1000 * _streamReloadFactor[quality])) {
